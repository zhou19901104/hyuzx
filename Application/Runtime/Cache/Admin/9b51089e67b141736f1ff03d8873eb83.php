...
<tr>
            <td colspan="10" class="table_title">
               案例管理<span class="fr">
                    <a href="<?php echo U('/Admin/Case/case_add');?>">添加案例</a>
                </span>
            </td>
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class='ol'>
				...
				<td align='center'>
                    <a href="<?php echo U('/Admin/Case/case_edit/',array('id'=>$vo['id']));?>">修改</a>
                    | <a href="javascript:void(0)" onclick="return confirmurl('<?php echo U('/Admin/Case/case_del/',array('id'=>$vo['id']));?>','确定删除该案例吗?')">删除</a>
                </td>
